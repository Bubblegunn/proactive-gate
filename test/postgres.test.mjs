...
const loadPool = async () => (await import("pg")).Pool;
...
storeContract(
  ...
  async (clock) => {
    if (!connectionString)
      throw new Error("PROACTIVE_GATE_POSTGRES_URL is not set");
    ...
    const store = new PostgresStore(
      {
        query: (text, params) =>
          pool.query(text, params ? [...params] : undefined),
      },
      clock,
    );
    return {
  ...
  },
  {
    skip: connectionString
      ? undefined
      : "PROACTIVE_GATE_POSTGRES_URL is not set",
  },
);

test(
  "PostgresStore: concurrent construction is race-safe",
  {
    skip: connectionString
      ? undefined
      : "PROACTIVE_GATE_POSTGRES_URL is not set",
      ...
      for (let round = 0; round < 15; round++) {
        const stores = Array.from(
          ...
          () =>
            new PostgresStore({
              query: (text, params) =>
                scoped.query(text, params ? [...params] : undefined),
            }),
        );
...
test(
  "PostgresStore: expired rows are swept on writes",
  {
    skip: connectionString
      ? undefined
      : "PROACTIVE_GATE_POSTGRES_URL is not set",
  ...
  async () => {
    ...
    try {
      ...
      const count = await scoped.query(
        "SELECT COUNT(*)::int AS count FROM proactive_gate_store",
      );
